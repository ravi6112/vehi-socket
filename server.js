...
// (async () => {
//   const channel = agServer.exchange.subscribe('updateChannel');
...
(async () => {
  for await (let {socket} of agServer.listener('connection')) {
    ...
    console.log('soocket is connected to backend');
    (async () => {
      // Set up a loop to handle and respond to RPCs.
      for await (let request of socket.procedure('updateProc')) {
        console.log('request data is coming over here');
        console.log('Client Id: ', request.data);
        agServer.exchange.transmitPublish(
          `updateChannel`,
          `this the data from server side server.js`
        );
      }
